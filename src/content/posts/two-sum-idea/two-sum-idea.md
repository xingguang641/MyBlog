...
当我们遍历到位置 right 时，我们需要查哈希表里有没有人 “期待” 一个值等于 $pre[right]$ 的前缀和；之后，再把未来的期望值 $pre[right] + k$ 存进去，表示 “我现在的前缀和，希望某个未来的位置能遇到这个数来凑成区间和 $k$” 。
...
* **如果 $pre[i] \ge 0$**
  ...
  根据前缀和的单调连续性，前缀和要想从 0 下降到 $pre[i]$（例如 -5），必然要在更早的位置先经过 $pre[i] + 1$（例如 -4）。也就是说，$pre[i] − 1$ 首次出现的位置，一定早于 $pre[i] − 2$ 、$pre[i] − 3$ 等更小数值首次出现的位置。因此为了让 $right - left$ 最大，我们 **只需寻找 $pre[i] - 1$ 第一次出现的位置** ，无需遍历所有小于 $pre[i]$ 的数值。
