...
选择最佳的损失函数时，请您关注您希望模型该如何处理离群值。例如：**MSE** 会使模型更接近离群点，而 **MAE** 则不会。因为与 **L1** 损失相比，**L2** 损失对离群值的惩罚要高得多。

:::important
**选择 MSE**：

- 如果您想严厉惩罚大误差，则用 MSE 效果更佳。
- 如果您认为离群值很重要，并表明模型应当考虑真实数据的方差。

**注意**：MSE 的数学属性通常会使优化更顺畅。均方根误差 (RMSE) 通常用于将误差恢复为与标签相同的量纲。

**选择 MAE**：

- 如果您的数据集中存在您不希望对模型产生过大影响的离群值，用 MAE 会更稳健。
- 如果您希望损失函数能更直接地展示出平均误差这一信息。

在实践中，选择的指标还可能来自于具体的业务问题。
:::
...
线性模型的损失函数始终会生成**凸面**。根据这一属性，当线性回归模型收敛时，我们可以知道该模型已找到能产生最低损失的权重和偏差。

也就是说，线性模型的损失函数有且仅有一个极小值点。

## 超参数

人为调控的参数称为超参数

**bath**：较大的 bath 有助于减少数据中存在的离群值所带来的负面影响。
