...
FF 有 n 只狗狗，编号为 $1, 2, 3, \ldots, n$ 。起初，狗狗们按编号升序排列。它们正在玩一个交换游戏，规则如下：

1. 狗狗们 **每秒** 都会进行一次交换。

2. **如果当前秒数是奇数**：交换下标为 $2 i - 1$ 和 $2i$ 的狗狗（其中 $1 \leq i \leq \lfloor \frac{n}{2} \rfloor$ ）。
* *例如 $n = 5$ ，第 1 秒时：$(1, 2)$ 交换，$(3, 4)$ 交换，序列变为 $2, 1, 4, 3, 5$* 。

3. **如果当前秒数是偶数**：交换下标为 $2 i$ 和 $2 i + 1$ 的狗狗（其中 $1 \leq i \leq \lfloor \frac{n - 1}{2} \rfloor$ ）。
...
Bob 对最大公约数（GCD）非常感兴趣。对于一组正整数 $(a_1, \ldots, a_k)$ ，$gcd(a_1, \ldots, a_k)$ 定义为最大的正整数 $d$ ，使得 $d$ 能整除每一个 $a_i$ 。
