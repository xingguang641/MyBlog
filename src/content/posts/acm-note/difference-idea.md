...
原数组可以通过对差分数组求前缀和恢复出来。差分的关键优势在于，它允许我们在只修改两个位置的前提下完成对整个区间的更新。例如，如果我们想让区间 $[l, r]$ 的所有元素都增加 $x$ ，在差分数组中仅需执行 `d[l] += x` 和 `d[r+1] -= x` 。之后在最终阶段对差分数组求前缀和，就能得到修改后的完整数组。

差分非常适用于处理大量区间加减操作的情形，因为它将每次区间更新的时间复杂度从线性的 $O(n)$ 降到常数级 $O(1)$ 。在算法设计中，差分常用于批量区间修改、构造前缀和结构、处理扫描线类问题等，是一种简洁而高效的基础技巧。

## 差分的广义视角

不过，我们今天讨论的并不是常见的 “差分数组” 这种标准技巧。经典差分技巧的用途非常明确：把区间修改变成端点操作，再用前缀和恢复结果。而在竞赛题中，我们有时候也会把一些做法称为 “差分思路”，但这其实并不是一个严格的理论概念，而更像是一类经验性的处理方式。

竞赛里的这类 “差分技巧” 通常不会真的去构造差分数组，而是利用 **相邻状态之间的变化** 来寻找突破口：有时是通过比较前后两个位置的差异来判断某个贡献何时开始或结束；有时是用 “多出来” 或 “少出去” 的量来简化区间统计；也可能是借助变化点来压缩状态、减少枚举或让转移更容易实现。它并不具备统一的形式，本质就是抓住 “变化” 这个切入口，把一个原本看起来需要全局处理的问题拆成若干局部操作。虽然抽象，但在一些结构复杂的题目中往往非常有效。
