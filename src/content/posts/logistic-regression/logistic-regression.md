...
与线性回归不同，逻辑回归的损失函数用的是交叉熵损失而不是均方损失，其中一个原因便是在逻辑回归中交叉熵损失函数的图像要比均方损失函数的图像要光滑很多（便于梯度下降），关于其他的一些原因可以观看下面这个视频进行了解。
...
把 $$\hat{y} = \sigma(Xw)$$ 带入上面的式子便可以直接得到上面代码中的公式了。
...
[【机器学习】线性回归和逻辑回归有什么区别？](https://blog.csdn.net/m0_74882984/article/details/144119791)
