...
在模式识别问题中，我们通常关心的是 **分类任务** ，例如判断一个人是否患有某种疾病。这时就不能简单地使用线性回归模型来解决。为此我们引入了一个非线性激活函数 $$g: \mathbb{R} \to (0,1)$$ 来预测类别标签的后验概率 $$p(y = 1 \mid x)$$ ，其中 $$y \in \{0, 1\}$$ ，函数 $g$ 的作用是把线性函数的值域从实数区间挤压到 0 和 1 之间。
