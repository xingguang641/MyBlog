...
$$
P = \sum_{i = 1}^{n} \max \big(0, \, Diff[i] \big) \quad N = \sum_{i = 1}^{n} \max \big(0, \, -Diff[i] \big)
$$
...
- 第一行包含两个整数 $n$ 和 $m$ ，分别表示序列长度和可操作的区间个数。
- 第二行包含 $n$ 个整数，表示序列 $A_1, \, A_2, \, \dots, \, A_n$ 。
- 接下来 $m$ 行，每行包含两个整数 $L_i$ 和 $R_i$ ，表示第 $i$ 个区间的左右边界。
...
- 接下来 $M$ 行，每行包含四个整数 $l$ 、$r$ 、$s$ 和 $e$ ，表示一次区间修改操作。
...
- 接下来 $m$ 行，每行包含三个整数 $l_i$ 、$r_i$ 和 $k_i$ ，表示一次区间修改操作。
...
- 接下来 $k$ 行，每行包含三个整数 $x_i$ 、$y_i$ 和 $p_i$ ，表示一次清理操作。
...
- 第一行包含一个整数 $n$ 。
- 接下来 $n$ 行，每行包含 $6$ 个整数，表示第 $i$ 个骰子的面数值。
