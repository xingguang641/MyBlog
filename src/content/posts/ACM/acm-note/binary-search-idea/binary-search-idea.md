...
与中位数类似，**平均数最大化或最小化** 的问题也可以通过 **二分答案法** 来解决。设数组为 $A_1,A_2,\ldots,A_n$ ，如果题目要求在某种约束条件下找到 **最大平均值** ，我们通常并不会直接计算平均数，而是对答案 $x$ 进行二分。对于每一个候选值 $x$ ，我们只需要判断是否存在一个合法的选择方案，使得该方案的平均值 **不小于 x** 。平均值问题的关键在于一个常见的代数变形。若某个集合 $S$ 的平均值满足：
