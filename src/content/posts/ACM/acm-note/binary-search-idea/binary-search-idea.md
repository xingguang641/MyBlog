...
该问题的背景是经典的 **画匠问题** ，而画匠问题是 **最大值最小化领域** 中最具代表性的经典案例。这类问题的核心挑战在于，如何在多名画匠协作且每人只能粉刷连续木板的前提下，让耗时最长的那位画匠工作量尽可能小。这一问题的标志性解法正是 **二分查找** 。之所以能够应用二分，是因为我们巧妙地利用了一个 **决策函数** ，将复杂的 **最优化问题** 转化为了一个具备严格单调性的 **判定问题** 。
...
换句话说，这是由所有 $0 \leq i \leq j < nums.length$ 的 `distinct(nums[i..j])` 组成的递增数组。其中，`distinct(nums[i..j])` 表示从下标 $i$ 到下标 $j$ 的子数组中不同元素的数量。
...
给定一个 $H \times W$ 的网格，每个单元格 $(i, j)$ 上都有 $A_{i, j}$ 枚硬币。高桥（Takahashi）初始位于 $(1, 1)$ ，手中持有 $x$ 枚硬币。
...
3. **移动**：如果 $k < H + W - 1$ ，高桥可以选择向下移动或向右移动。如果 $k = H + W - 1$ ，他停留在当前位置。
...
- 最后一行包含 $H+W-1$ 个整数，表示每天需消耗的硬币数量 $P_1, P_2, \ldots, P_{H+W-1}$ 。
