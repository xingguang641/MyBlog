...
从代码编写的视角审视，这四类变体在 **初始化逻辑** 、**迭代步长** 以及 **区间折半** 的计算方式上表现出高度的同质化，整体框架几乎完全一致。这种结构上的高度统一意味着我们只需掌握一套 **核心模板** ，即可通过修改局部参数来适配不同的检索需求。其本质的差异化特征主要集中于两点：**判定条件的具体选择** ，以及在条件达成时 **对搜索方向（左、右边界）的收缩决策** 。正是这些看似细微的逻辑差异，决定了算法指针最终会向哪一类 **临界位置** 进行偏移。
...
为了快速完成统计，我们可以先对数组进行排序。在有序序列中，利用 **双指针** 技巧可以在 $O(n)$ 的时间内扫描出所有满足条件的数对，而无需嵌套循环。通过在 $0$ 到数组最大差值之间进行二分，配合这个高效的计数函数，我们就能在对数时间内精准锁定第 $k$ 小的数对距离。这种将序数查找转化为值域判定的方式，是解决此类问题的标准思路。
