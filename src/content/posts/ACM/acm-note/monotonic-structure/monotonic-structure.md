...
我们经常会遇到需要处理所有子数组的一类经典问题，解决这类问题最直接的思路是枚举所有可能的子数组，然后针对每个子数组单独处理。然而枚举所有子数组的时间复杂度较高，在数据规模较大时无法满足要求。

滑动窗口是针对这类问题的经典优化方法，其核心是在数组中维护一个连续区间，通过不断调整区间的左右端点以保证区间在移动过程中不会发生回退，从而避免对所有子数组进行枚举，将 $O(n^2)$ 的子数组问题优化至 $O(n)$ 。

但是，滑动窗口并不是一种通用的区间优化技巧，并非所有子数组问题都可以直接使用。其能够成立的关键在于题目中存在某种**单调性**，使得窗口向某个方向扩展或者收缩时，状态变化具有确定的趋势。

因此，滑动窗口问题通常会利用子数组本身的限制条件，通过判断当前窗口是否合法，排除大量无需枚举的状态。正是这些限制条件提供了窗口移动过程中的单调关系，使得左右指针能够不断向前移动，而无需进行回退操作。

以最常见的「子数组累加和不超过 (K)」为例。假设数组中的所有数字均为正数，当固定左指针 (left)，不断移动右指针 (right) 时，窗口内的累加和会随着 (right) 的增加不断增大。当累加和超过限制时，继续向右扩展必然无法满足条件，因此在当前 (left) 固定的情况下，后续的 (right) 均不需要继续枚举。

随后移动 (left) 缩小窗口，并继续寻找满足条件的区间。由于数组中的元素均为正数，去掉左端点只会使窗口累加和减小，因此原本合法的右端点仍然可以继续使用，(right) 指针无需回退。

正是由于这种单调性，滑动窗口中的两个指针均只会单向移动，每个元素最多进入窗口一次、离开窗口一次，最终将原本的双重循环优化为线性复杂度。

根据窗口限制条件的不同，滑动窗口可以进一步划分为定长滑动窗口和不定长滑动窗口。上述通过窗口内部性质进行约束，并根据约束条件动态调整窗口大小的问题，本质上属于**不定长滑动窗口**。这类问题的窗口大小并不由长度直接决定，而是随着窗口状态变化不断调整，通常用于求解满足约束条件的区间性质。其中限制条件通常可以分为「至多」和「至少」两类，而「恰好」条件一般也可以通过转化得到。

而当限制条件本身与窗口长度相关时，则属于**定长滑动窗口**。这类问题直接维护一个固定大小或大小受到长度约束的区间，重点在于维护窗口内部的信息。需要特别注意的是，窗口长度「不超过 (K)」这类限制并不属于不定长滑动窗口。虽然窗口大小并非固定值，但其约束条件本质上仍然来自长度限制，因此仍属于定长滑动窗口。真正的不定长滑动窗口，其窗口变化必须由窗口内部状态决定，而不是由区间长度本身决定。

因此，判断一个问题是否能够使用滑动窗口，关键并不在于它是否涉及子数组，而在于能否找到窗口状态变化中的单调关系。只有利用这种单调性，才能跳过无需枚举的情况，使滑动窗口真正达到降低复杂度的目的。

### 种类滑窗相关问题

约束条件不固定，比如要求窗口内每个元素的个数都要超过k个，由于窗口内的元素种类不知道，因此究竟有多少个约束也不知道，是不固定的，没办法确保窗口一定满足所有约束，因此需要固定窗口元素，这种滑窗叫做种类滑窗，通过固定窗口元素来解决上述问题。

并且这种题一般不是数组问题，而是字符串问题，字符串和数组没有本质区别，唯一区别是字符集大小，由于种类滑窗需要枚举窗口内部的元素种类，因此字符集大小不能太大，否则枚举代价也过大，因此一般都是用字符串来出题，避免枚举代价过大。

### 分组滑窗相关问题

窗口限制单一元素，因此可以将每种元素单独拎出来进行滑窗，也就是分组执行滑窗。
