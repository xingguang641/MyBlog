...
给定整数 $C$（ $1 \leq C \leq n$ ）。对于 $i = 0, 1, \ldots, M-1$ ，定义 $X_i$ 如下：高桥从点 $(i + 0.5)$ 开始沿顺时针方向移动。只要遇到的总人数少于 $C$ ，他就会继续移动；一旦遇到的总人数达到或超过 $C$ ，他就会立即停止。在此过程中，遇到的总人数即为 $X_i$ 。如果停止点有多个人，他们都会被计入。请计算：
