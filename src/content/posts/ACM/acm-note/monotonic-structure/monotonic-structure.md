...
定长滑动窗口：包含真的定长滑窗，以及有大小限制的某个区间（限制区间大小不超过K不是不定长滑窗，本质也是定长，只有拿其他条件进行约束的窗口才是不定长，直接拿长度来约束的不是不定长）。主要用于维护某个区间内的东西，重点在维护。

不定长滑动窗口：求解子数组的最长长度、最短长度、个数等等。子数组一定有约束，约束一定是有限的，有至多条件也有至少条件，恰好条件能转化成这两个。条件跟求解目标相对应，比如累加和不超过k的最长子数组，不会出现累加和不超过k的最短子数组这种问题。
...
3. [【Jerrycyx】实用而好写的数据结构](https://www.cnblogs.com/jerrycyx/p/18683014)
