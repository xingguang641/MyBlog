...
我们不妨以第二种形态，即 `010101` 作为唯一的全局参考基准线。这意味着在理想的基准线上，所有偶数下标对应的字符应当为 `'0'` ，所有奇数下标对应的字符应当为 `'1'` 。此时，我们将倍增字符串中的每一个字符与该基准线进行比对。如果当前字符不满足这个奇偶交替的规律，我们就将其视为一个错位点，也就是我们需要进行类型 $2$ 反转操作的位置。
...
- 第二种情况，当子串左端点下标是奇数时。因为左端点在全局基准线上变成了奇数位置，这导致整个窗口内部的奇偶性相对于全局基准线 **整体向右偏移 1 位** 。这意味着如果我们想把子串变成 `010101` 形态，那些 **符合** 全局基准线的字符就需要反转；如果我们想把它变成 `101010` 形态，那些 **不符合** 全局基准线的字符就需要反转。
...
这种思维方式的本质是 **升维思考与降维打击** 。通过引入字符种类数这一额外的逻辑维度，我们将一个看似不具备单调性的全局搜索问题，转化为了若干个局部单调的子问题。在每一次子问题的求解过程中，由于种类数被严格限制，窗口的收缩逻辑变得清晰且唯一，从而确保了双指针移动的有效性与正确性。
