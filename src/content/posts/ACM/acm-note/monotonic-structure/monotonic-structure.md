...
前面介绍的滑动窗口之所以能够成立，是因为窗口需要满足的限制条件在整个移动过程中保持不变。虽然窗口的左右端点不断移动，窗口内部状态也会随之变化，但判断窗口是否合法的标准始终不会发生变化。



不过有些问题的限制条件会随窗口状态变化。有一类条件是「窗口内每种元素的出现次数都不少于 K」，这类条件中窗口包含哪些元素并不确定，需要满足的条件也无法在移动前确定。

可以把未满足的条件看作未完成任务。窗口向右扩展时，如果新加入的元素已经在窗口中，它的次数增加，可能让一个任务完成，而如果这个元素此前不在窗口中，窗口内就多出一种元素，同时多出一个未完成任务。因此未完成任务数在扩展时可能增加也可能减少，不具备单调性，这样就不能像普通滑动窗口那样根据当前窗口状态直接决定指针移动方向，窗口收缩时也同样存在这个问题。

进一步看，限制条件变化的根本原因是窗口中元素种类数量不确定，种类数量变化会使需要满足的条件数量变化，解决的关键是先消除这种变化，即枚举窗口中包含的元素种类数量。种类数量固定后，需要满足的条件数量也随之固定，原本随窗口状态变化的限制条件就被转化为确定限制条件，之后再对每一种固定的种类数量使用滑动窗口进行维护，就能重新利用滑动窗口的单调性完成求解。
