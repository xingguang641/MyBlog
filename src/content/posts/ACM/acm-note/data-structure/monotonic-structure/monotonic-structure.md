...
不妨将第二种形式作为全局参考基准线，这意味着在目标基准线上，所有偶数下标对应的字符应当为 `'0'` ，所有奇数下标对应的字符应当为 `'1'` 。此时我们将倍增字符串中的每一个字符与该基准线进行比对，如果当前字符不满足这个奇偶交替的规律，我们就将其视为一个错位点，也就是需要进行类型 $2$ 反转操作的位置。
...
当问题同时受到两种约束限制时，候选值之间往往会因为这两种约束产生明显的支配关系。假设有一批物品，每件物品都有容量和价值两个属性，现在要求选择两个物品，使它们的总容量不超过 $m$ 且总价值尽可能大。对于这两个物品而言，如果其中一个物品的容量更大的同时价值更低，那么它在任何情况下都不可能成为更优的候选值，因此可以直接排除。将物品按照容量排序后，这种候选值之间的相互排除关系就可以通过单调结构进行维护。

这种候选值之间的相互排除关系，正是单调堆栈能够发挥作用的基础。当候选值之间存在明确的大小关系时，我们就可以利用这种关系不断排除已经失去竞争力的候选值，并将剩余候选值维护成单调结构。**最近上/下邻问题** 和 **最远上/下邻问题** 都可以利用候选值之间的大小关系解决，下面将分别从这两类问题展开进行分析。
...
从更宏观的角度来看，由于最远上下邻问题中的候选元素一旦加入单调栈后便不会被剔除，因此这个单调栈本质上是在不断记录历史上的最大值。为了进一步简化代码实现，我们可以预处理一个前缀最大值数组：
