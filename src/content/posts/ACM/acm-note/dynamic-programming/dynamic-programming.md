...
仔细观察这个状态可以发现，对于固定的 $u$ 和 $i$ ，如果某个 $j$ 可以满足条件，那么更大的 $j$ 也可以满足条件，因此只需要记录满足条件且最小的右偏移量 $j$ 。于是可以将 $j$ 这一维压入状态值中：
...
对于相同的 $S$ 而言，较大的 $cur$ 在后续转移中始终能够得到更大的结果，因此只需要记录最大的 $cur$ 以及对应的方案数。于是可以将 $cur$ 这一维压入状态值中：
