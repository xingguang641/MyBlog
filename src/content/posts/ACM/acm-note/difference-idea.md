...
从数学本质上看，差分可以被视为 **离散意义下的导数** 。连续数学利用导数刻画函数的变化速率，离散数学则利用差分刻画数据的变化趋势：

$$
diff[i] = arr[i] - arr[i-1]
$$

这一映射关系在逻辑上对应了连续函数中导数的离散近似：

$$
f'(x) \approx f(x) - f(x-1)
$$

在这种视角下，原数组描述系统的状态，而差分数组则描述状态的变化。对差分数组求前缀和，本质上等同于 **对离散导数进行积分** 。差分操作与前缀和操作互为逆运算，是处理区间修改与静态查询问题最常用的方法。

对于多维差分而言，我们既可以通过容斥原理进行推导，也可以基于线性叠加原理进行构建。接下来我们将分别从这两种视角出发，逐步拆解多维差分的数学本质。

由于直接推导多维差分的通用公式难度较大，我们先以二维差分作为切入点。若将状态数组中的每个元素看作一个矩阵区域，则坐标 $(x, y)$ 处的差分值本质上就是从 $(x-1, y-1)$ 到 $(x, y)$ 这一网格单元所构成的矩形面积。根据容斥原理，可以得到：

$$
diff[x][y] = arr[x][y] - arr[x-1][y] - arr[x][y-1] + arr[x-1][y-1]
$$

以此类推，对于 $n$ 维空间中的状态数组 $arr$ ，其在坐标 $x = (x_1, x_2, \dots, x_n)$ 处的差分通项公式为：

$$
diff[x_1][x_2]\dots[x_n] = \sum_{k=0}^{n} (-1)^k \sum_{1 \leq i_1 < i_2 < \dots < i_k \leq n} arr[x_1]\dots[x_{i_1}-1]\dots[x_{i_k}-1]\dots[x_n]
$$

而从线性叠加原理的视角来看，多维差分在本质上可以拆解为不同维度上一维差分操作的自由复合。由于运算满足交换律与结合律，各个维度之间的计算顺序完全无关。这意味着我们无需跨越到高维空间去进行复杂的几何想象，只需对数组的每一个维度依次独立执行一维差分，最终也能直接构建出相同的多维差分数组。
