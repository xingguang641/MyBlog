...
$$
O(N \cdot (N + \text{maxVal})) = O(N^2 + N \cdot \text{maxVal})
$$
...
* **构建左侧有序集合 left**：如果当前遍历到的下标 $idx[i]$ 小于 $j$ 的原始下标，且满足 $|arr[idx[i]] - arr[j]| \leq a$ ，则将其数值加入 `left` 。
* **构建右侧有序集合 right**：如果 $idx[i]$ 大于 $j$ 的原始下标，且满足 $|arr[idx[i]] - arr[j]| \leq b$ ，则将其数值加入 `right` 。
