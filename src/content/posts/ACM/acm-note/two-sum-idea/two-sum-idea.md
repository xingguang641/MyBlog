...
不仅如此，我们还可以从 “中间位置” 入手进行划分。由于题目通常要求满足 $i < j < k$ ，一旦固定了中间位置 $j$ ，原问题就被自然地拆分为左右两个互不干扰的区间：左侧 $[0, j-1]$ 与右侧 $[j+1, n-1]$ 。此时，我们可以分别在两侧维护所需的信息，例如 **对两边进行排序** 或预处理，从而将跨区间的匹配问题转化为更易处理的形式。这种处理方式本质上借鉴了[归并分治思想](https://xingguang641.com/posts/acm/acm-note/merge-sort/merge-sort/)，类似于归并过程中对左右子数组的独立处理与再合并，使问题结构更加清晰，也为进一步优化提供了空间。
...
即可在极短时间内获取满足双重约束的 $i$ 的数量。

这种设计巧妙地利用了指针移动的自然时序，实现了历史状态的自动过滤与复用。它将原本相互耦合的三维约束，简化为 “范围检索” 与 “动态入库” 的循环，在值域范围较小（如本题的 $1000$ ）时，相比暴力枚举具有显著的性能优势。
