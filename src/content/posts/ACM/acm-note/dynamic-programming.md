...
在动态规划中，建模的核心难点往往不在于推导转移公式，而在于对 **状态的精准刻画** 。状态设计的本质是信息的一场取舍游戏：它决定了哪些关键变量必须被显式记录以维持逻辑完备，哪些冗余信息可以被忽略以降低搜索开销。一个精妙的状态定义能让转移路径清晰可见，而一个臃肿或错位的定义则会使问题陷入复杂度爆炸或逻辑断层的泥潭。
...
你正在面对 $n$ 只怪兽，必须按 **从左到右** 的顺序依次通过它们。每只怪兽有两个属性：防御力 $d_i$ 和贿赂它所需的金币数 $p_i$ 。

对于每一只怪兽，你可以选择以下两种方式之一通过：

1.  **贿赂（雇佣）**：支付 $p_i$ 个金币。贿赂成功后，这只怪兽会加入你的队伍，你的 **总能力值** 增加 $d_i$ 。
2.  **武力打败**：如果你的 **当前总能力值** 大于该怪兽的防御力 $d_i$ ，你可以选择直接打败它。在这种情况下，你不需要支付金币，但你的总能力值也 **不会增加** 。

你的目标是按顺序通过所有 $n$ 只怪兽。请计算通关所需的 **最少总金币数** 。
...
- 第一行包含一个整数 $n$，表示怪兽的数量。
- 第二行包含 $n$ 个整数，表示每只怪兽的防御力 $d_1, d_2, \ldots, d_n$ 。
- 第三行包含 $n$ 个整数，表示贿赂每只怪兽所需的金币 $p_1, p_2, \ldots, p_n$ 。
...
这种技巧在 **多路径同步移动** 、**网格路径相遇** 以及 **多对象追踪** 等问题中尤为常见。例如，在两个棋子同时从网格左上角出发移动到右下角的问题中，如果它们每步都只能向右或向下走，那么在任何时刻，两个棋子的坐标 $(x_1, y_1)$ 和 $(x_2, y_2)$ 必须满足步数守恒：$x_1 + y_1 = x_2 + y_2 = step$ 。此时，我们只需记录当前的步数 $step$ 以及两个棋子的横坐标 $x_1$ 和 $x_2$ ，纵坐标则可以通过 $y = step - x$ 直接推导而出。

综上所述，自由度压缩不仅是一种节省空间的技巧，更是一种对问题 **结构特征** 的深度洞察。它要求建模者跳出 “直观记录” 的思维定式，转而寻找变量背后的 **内在一致性** 。通过减少状态的自由度，我们可以将原本看似不可做的复杂问题，转化为结构精炼、运行高效的低维动态规划模型。
...
虽然朴素递归能完整描述求解逻辑，但其最大的弊端在于产生大量的 **重复子问题** 。在递归树的展开过程中，不同分支往往会多次触达完全相同的状态。当状态空间有限且计算结果具有确定性时，我们可以引入缓存机制，即 **记忆化搜索（Memoization）**。通过记录已求解状态的结果，计算效率可以实现从指数级到多项式级的质变。在这个意义上，记忆化搜索正是 **自顶向下（Top-down）**实现动态规划的核心手段。

当状态间的依赖关系（即状态转移）被进一步明晰后，如果能建立一套满足依赖约束的计算顺序，递归过程便可转化为 **自底向上（Bottom-up）**的迭代形式。此时，递归调用中隐含的逻辑依赖被显式地表达为数组间的 **状态转移方程** ，计算过程则严格按照拓扑序稳步推进。若将每个状态视为图中的节点，状态转移则构成了一张 **有向无环图（DAG）**。递归本质上是在该图上进行深度优先遍历，而迭代动态规划则是按照拓扑序进行系统性的扫表计算。因此，二者 **在数学本质上是高度统一的，差异仅在于执行的路径与形式** 。

综上所述，**动态规划并非独立于递归之外的孤立技巧** ，而是对递归求解过程的结构化与系统化优化。一个严谨的建模路径通常遵循：首先利用递归明确状态与转移逻辑；其次通过记忆化消除冗余计算；最后根据依赖关系重构为自底向上的迭代实现。这一方法论路径不仅确保了模型构造的正确性，也为算法从抽象逻辑到高效实现的转化提供了清晰的指引。
...
象棋骑士有 8 种可能的移动方式。每次移动在 $L$ 形方向上前进：选择一个方向（上下左右）走 2 格，然后垂直于该方向走 1 格。

每当骑士需要移动时，它会从 8 种可能的移动中 **等概率** 地选择一种（即使棋子移动后会离开棋盘），然后移动到那里。
...
给出两个长度为 $n$ 的排列 $P_1$ 和 $P_2$ ，计算它们的最长公共子序列（Longest Common Subsequence, LCS）的长度。
...
1. [【ACM 算法题单】划分动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/partition-dp/)

2. [【ACM 算法题单】背包动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/knapsack-dp/)

3. [【ACM 算法题单】区间动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/interval-dp/)

4. [【ACM 算法题单】树型动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/tree-dp/)

5. [【ACM 算法题单】状压动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/tree-dp/)

6. [【ACM 算法题单】数位动态规划相关问题](https://xingguang641.com/posts/acm/acm-type/dp-classification/tree-dp/)