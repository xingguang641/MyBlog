...
**划分动态规划（Partition Dynamic Programming）**是一类重要的动态规划模型，其基本思想是：将一个序列或区间按照某种方式划分为若干连续的部分，通过组合各部分的最优解得到整体的最优解。这类问题通常具有明显的 “分段” 结构，例如在序列中插入若干个分隔符、将区间划分为若干段、或者逐步决定最后一个划分位置等。由于每一种划分方式都可以看作是由前一部分的最优解扩展而来，因此非常适合用动态规划进行求解。
...
另外，当状态转移方程可以变形为具有线性结构的形式时，还可以使用 **斜率优化（Convex Hull Trick）**。如果转移能够表示为

$$
dp[i] = \min (dp[k] + A_k \times B_i + C_k)
$$

这样的结构，就可以把每一个候选状态看作一条直线，并利用凸包维护最优决策点，从而避免对所有 $k$ 进行枚举。通过这种方法，原本需要 $O(n^2)$ 的转移过程可以降低到 $O(n)$ 或 $O(n\log n)$ 。斜率优化在分段代价为二次函数或线性组合的划分问题中应用较为广泛。
