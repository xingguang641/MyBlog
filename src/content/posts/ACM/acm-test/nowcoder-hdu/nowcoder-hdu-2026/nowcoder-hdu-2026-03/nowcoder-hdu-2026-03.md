...
tags: [Algorithm, Contest, Nowcoder]
category: ACM Test
draft: false
---
...
这道题的关键在于观察 $f(x)$ 的含义。一个连续的 $1$ 段都有唯一的起点，如果从二进制的高位向低位观察，那么每个连续的 $1$ 段都对应一个 `'01'` 的边界。例如 $23 = (10111)_2$ 中有两个连续的 $1$ 段，也对应两个 `'01'` 的边界。

在二进制的最高位和最低位两侧都补一个 $0$ 后，每个连续的 $1$ 段都会对应一个 `'01'` 边界，同时也对应一个 `'10'` 边界，因此我们可以通过统计相邻位中 `'01'` 或 `'10'` 出现的次数来得到 $f(x)$ 。

于是对于每个相邻的位 $j$ 和 $j + 1$ ，只需要统计所有数字在这两位上的组合情况。两位一共有四种状态：

$$
00 \quad 01 \quad 10 \quad 11
$$

设 $cnt[j][s]$ 表示当前所有数字中，第 $j$ 位和第 $j + 1$ 位组成状态 $s$ 的数量，其中 $s$ 分别对应上述四种情况。那么当前所有数字的 $\sum f(a_i)$ 就等于所有位置上 `'10'` 状态的数量之和：

$$
ans = \sum_{j = 0}^{29} cnt[j][10]
$$

接下来考虑一次操作，对于固定的相邻位 $j$ 和 $j + 1$ ，经过操作后的状态只与原来的状态以及 $x$ 在这两位上的取值有关。因此可以枚举四种原状态，分别计算操作后对应的新状态，并将原状态的数量转移到新的状态中。

初始化时枚举每个 $a_i$ 的 $30$ 对相邻位，统计对应的状态数量。每次操作完成后，统计所有 `'01'` 或 `'10'` 的数量。
...
- 对于每个测试用例：
    ...
    - 第一行包含三个整数 $n$ 、$s$ 和 $t$ 。
    - 第二行包含 $n - 1$ 个整数，表示在顶点 $i$ 与 $p_i$ 之间存在一条边。
