...
这道题的关键在于 $f(x)$ 的含义，一个连续的 $1$ 段都有唯一的起点，如果从二进制的高位向低位观察，那么每个连续的 $1$ 段都对应一个 `'01'` 的边界。例如 $23 = (10111)_2$ 中有两个连续的 $1$ 段，也对应两个 `'01'` 的边界。

在二进制的最高位和最低位都补一个 $0$ 后，每个连续的 $1$ 段都会对应一个 `'01'` 边界，同时也对应一个 `'10'` 边界，因此我们可以通过统计相邻位中 `'01'` 或 `'10'` 出现的次数来得到 $f(x)$ 。
