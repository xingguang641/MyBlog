...
最长递增子序列（LIS）是一个非常典型的 **偏序型动态规划问题** 。最自然的思路是从朴素状态设计入手：定义 $dp[i]$ 表示 **以第 i 个元素结尾** 的最长递增子序列长度。那么在计算 $dp[i]$ 时，我们只需要枚举所有 $j < i$ ，如果 $a[j] < a[i]$ ，就可以尝试用 $dp[j] + 1$ 来更新 $dp[i]$ 。这种做法本质上是在所有可能的前驱中寻找最优决策，因此时间复杂度为 $O(n^2)$ 。
