...
然而这种朴素做法虽然直观，但效率偏低。进一步观察可以发现：在位置 $i$ 之前的所有位置中，那些 **数值更大但对应 dp 值更小** 的位置，是不可能成为最优转移来源的。因为如果存在一个位置 $j_1$ ，它的数值更小且 dp 值更大，那么它一定比数值更大且 dp 更小的位置更优。这说明在所有历史状态中，实际上存在大量 “劣质状态” 是可以被提前排除的。
