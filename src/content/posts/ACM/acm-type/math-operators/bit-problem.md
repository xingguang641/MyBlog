...
总体来说，位运算问题往往遵循一个非常典型的解决框架：**先按位拆解问题，再分析每一位的可行性，最后通过贪心逐位构造答案** 。而像 01-Trie 这样的结构，则进一步将这种思路落地为高效算法，使得原本复杂的搜索过程变得清晰且可实现。这也是逐位拆解方法在算法竞赛中被广泛应用的重要原因。
...
进一步地，这种结构还带来了一系列重要性质。例如：

$$
a \oplus a = 0,\qquad a \oplus 0 = a
$$

这说明同一个数被异或两次会完全抵消，因此异或结果只与 **每个数出现的奇偶次数** 有关。这一性质在很多统计类问题中非常常见，例如通过对整个序列进行一次异或，可以快速找出只出现一次的元素。
...
因此一连串异或操作可以任意重排或合并，从而大大简化表达式结构。

从更抽象的角度来看，这种模 $2$ 加法使得异或运算构成了一个 **线性空间结构 GF(2)** 。借助这一结构，可以解决许多更复杂的问题，例如判断某些数是否可以由其他数异或表示，或者维护所有可表示的异或结果等，从而避免对状态进行暴力枚举。

在实际问题中，这种加法结构还会带来一些非常经典的技巧。例如可以定义 **前缀异或和**：
...
这一形式与普通前缀和完全类似，因此许多区间异或问题都可以通过前缀异或在 $O(1)$ 时间内计算。

另外，由于异或具有可逆性：

$$
a \oplus b = x \iff b = a \oplus x
$$

在许多问题中，可以将 “寻找满足某种异或关系的数对” 转化为寻找对应的 **目标值** 。这种结构与经典的[两数之和思想](https://xingguang641.com/posts/acm/acm-note/two-sum-idea/two-sum-idea/)非常相似：在遍历数组时，可以用哈希表记录已经出现的元素，并查询当前元素对应的目标值 $a_i \oplus x$ 是否存在。如果存在，就找到了一组满足条件的数对。这种方法在数组与区间相关的异或问题中非常常见。
...
与按位与、按位或不同，按位异或运算并不具备明显的单调性，因此很多问题往往无法通过简单的 “不断变大” 或 “不断变小” 的规律来进行分析。许多看似复杂的异或问题，如果只从数值变化的角度出发，往往难以找到清晰的思路。
...
也就是说，前缀异或的结果仅与 $n \bmod 4$ 有关，呈现出周期为 $4$ 的循环结构。因此，对于任意区间 $[l, r]$ ，都有：
