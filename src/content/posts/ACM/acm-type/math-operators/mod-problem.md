...
这种维度的拓展在序列问题中展现了极强的泛化能力。例如在处理 “子段和为 $M$ 的倍数” 或 “图论路径长度模 $M$ 为特定值” 的题目时，本质上都是在维护 **前缀和的同余状态** 。由于子段和可以表示为两个前缀和之差，问题便转化为了寻找满足 $S_i \equiv S_j \pmod M$ 的等价类对。通过在 DP 状态或图的节点中增加一维当前余数，我们将原本无法直接处理的区间约束转化为了同余系下的状态转移，这正是从 $O(N^2)$ 的暴力迭代迈向 $O(N \cdot M)$ 甚至 $O(N)$ 高效求解的逻辑跳板。
