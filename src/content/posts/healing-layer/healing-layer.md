...
老博客中使用的新浪微博图床图片无法显示，通常是因为新浪微博在某次更新后启用了 **防盗链（Referer Hotlinking Protection）** 机制。简单来说，当浏览器请求图片时，新浪服务器会检查 HTTP 请求头中的 `Referer` 字段（即请求来源）。如果 `Referer` 显示请求来源于你的个人博客域名，而非新浪自家域名，服务器就会拒绝访问并返回 403 错误。

![图片报错示例](src\content\posts\healing-layer\图片报错1.png)

由于过去新浪微博图片存储非常方便，许多老博客都会直接使用微博图床的图片链接。这也导致了大量老博客出现图片丢失的问题。解决这一问题的方法相对简单：通过浏览器插件修改请求头，将 `Referer` 伪装成新浪微博的域名，就可以 “欺骗” 服务器，使其正常返回图片。
