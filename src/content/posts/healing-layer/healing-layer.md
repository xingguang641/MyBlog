...
老博客中的新浪微博图床图片无法显示，通常是因为新浪微博开启了 **防盗链（Referer Hotlinking Protection）** 机制。

简单来说，当浏览器加载图片时，新浪服务器会检测请求头中的 `Referer` 字段（即请求来源）。如果该字段显示请求来自你的个人博客域名，而非新浪自家的域名，服务器就会拦截请求并返回 403 错误。
...
**关于匹配规则的说明：**
在 “匹配规则” 一栏中，你需要填入**图片链接的域名**（即图床服务器的域名）。
你可以按 `F12` 打开开发者工具，选中无法显示的图片，查看其 URL。例如，如果图片地址是 `https://ws1.sinaimg.cn/large/...`，那么你应该填入 `ws1.sinaimg.cn` 或 `sinaimg.cn`。
