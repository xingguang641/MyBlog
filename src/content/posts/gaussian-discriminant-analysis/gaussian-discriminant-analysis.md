...
接[上面](#似然函数)所说，我们要想满足先验，首先就要求解出四个参数。根据极大似然估计的原理，我们要想模型最优，我们只需要让似然函数取到最大值即可，也就等价于对数似然函数取到最大值。因此我们可以对四个关键参数求偏导来得到四个参数的具体值。
...
当 $ \Sigma_0 = \Sigma_1 = \Sigma $ 时，经过贝叶斯判别后得到的判别边界是：$w^{\rm T} x + b = 0$ ，也就是说，GDA 的决策边界是线性的（此时 GDA 约等于逻辑回归）。

但如果不强制协方差相等， GDA 的 **对数几率** 会出现二次项 $x^{\rm T} A x$ ，此时的模型叫做 **二次判别分析** （Quadratic Discriminant Analysis，简称 QDA）。此时它的决策边界将会是是二次曲线/曲面，而不是直线了。
