...
$$
P(x_i \mid c) = \frac{(\sum_i x_i)!}{\prod_i x_i!} 
...
$$
P(x_i \mid c) = \prod_i p_{c,i}^{x_i} (1 - p_{c,i})^{1 - x_i} 
\quad \text{where } 
...
$$
P(x|y) = \frac{\text{count}(x, y) + 1}{count(y) + n_i}
$$

其中 $N$ 是样本总数， $K$ 是类别数， $n_i$ 是特征 $x_i$ 的取值个数。

- 贝叶斯思想

有时候在训练集里，某个类别确实 **没有样本** ，那整个模型就会崩溃（虽然这很少发生）。此时进行平滑操作后模型的确可以在新数据中处理 “未见类别” 的情况（尤其是在增量学习或样本极端不均衡时）。

3. 如何直观理解对条件概率建模表示判别模型，对联合概率建模表示生成模型？（究竟什么是判别模型？什么是生成模型？为什么朴素贝叶斯对后验概率建模却是生成模型？）
...
因此生成模型是在建模 $P(x, y)$ ，但根据公式 $P(x, y) = P(x|y)P(y)$ 可以知道，建模后验概率就是在建模联合概率，因此 **建模后验概率也属于生成模型** （不如说建模联合概率本质就是在建模后验概率）。
