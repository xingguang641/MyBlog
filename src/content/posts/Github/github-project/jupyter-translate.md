...
Jupyter Translate 是一个能够自动翻译 `.ipynb` 文件的工具，并且支持多种翻译引擎，能够在保证代码框中的内容不变的情况下，批量处理 Markdown 文本与注释的语言转换。

通过简单的配置，可以让它在笔记中实现一键翻译，适用于整理学习资料、生成双语 Notebook 或快速阅读外文教程。下面将介绍如何安装、配置并使用这个工具。
...
该项目支持多种翻译后端，为了获得最佳的翻译质量，我们通常将 **大语言模型** 和 **阿里云机器翻译** 混合使用。为了接入具体的翻译模型，我们需要在项目中创建一个 `config.ini` 文件，然后将 `config.ini.example` 文件的内容复制过来，并填入对应的信息。
