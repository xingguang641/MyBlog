...
在个人开发中，Git 的核心骨架其实非常纯粹。你不需要复杂的团队规范，每天最核心的开发节奏就是基础的三个核心命令：将本地修改打包、归档并同步到云端。
...
只靠上面的主干三个核心命令虽然能跑通，但一旦遇到 “逻辑异常需要回退” 或者 “想同时做多个实验性尝试” 时就会产生局限。Git 所有的分支、回退机制，本质上都是为了让我们更便利地使用上面的基础核心命令。
...
有时候手太快执行了 `git add .` ，结果发现把一些垃圾临时文件、或者没写完的代码也一起打包进去了。

无需手动去删代码，直接在终端执行以下命令即可撤销暂存：
...
团队开发中严格禁止直接向主分支提交代码。所有新功能、需求或缺陷修复，都必须在独立的本地分支中进行完整的三连操作，完成后推送到远程仓库。
