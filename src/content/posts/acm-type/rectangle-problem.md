...
矩形累加和是矩形类问题中最基础、也最常见的一类题型。对于一个二维矩阵，最直接的思路是枚举矩形的左上角和右下角位置，从而确定一个子矩形并计算其元素之和。然而，这种朴素做法中，左上角的选择需要 $O(n^2)$ 的时间，右下角同样需要 $O(n^2)$ 的时间，整体枚举复杂度高达 $O(n^4)$ ，在实际题目中往往是 **无法接受的** 。
