...
## 从简单到多元（Univariate to Multivariate）
...
令误差项 $e_i = \hat{y}_i - y_i = (w x_i + b) - y_i$ ，损失函数可以写为 $\displaystyle L = \frac{1}{n}\sum e_i^2$ 。根据链式法则：
...
**在假设观测噪声服从高斯分布（正态分布）的前提下，对线性模型的极大似然估计。**
